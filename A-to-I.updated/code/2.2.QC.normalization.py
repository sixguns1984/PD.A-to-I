...
def main(ratio_file, pcs=50):
    ...
    for dic in stream_table(open(ratio_file),' '):
        ...
        chr_ = chrom.split(":")[0]
        if chr_ in 'XYM': continue ##romove chromosome X, Y, M
        ...
        for sample in header:
            ...
            num, denom = count.split('/') ## editing ratio split
            if float(denom) < 1:
                count = "NA"
                tmpvalRow.append("NA")
                NA_indices.append(i)
                ...
                count = (float(num)+0.5)/((float(denom))+0.5) ## prevent 0/x
        ...
        if np.std(valRow) < 0.005: continue ## liweimin
